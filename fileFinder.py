...
def file_search(target_dir_path, *args):
    ...
    match_path_list = map(lambda file: os.path.join(target_dir_path, file), match_name_list)
    ...
    sub_match_path_list = itertools.chain.from_iterable(map(lambda dir_path: file_search(os.path.join(target_dir_path, dir_path), *args), dir_path_list))

    # ファイル名に条件が含まれるファイルのパスを列挙
    return itertools.chain(match_path_list,sub_match_path_list)